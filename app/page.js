    ...
    <div onClick={() => onView(property)} style={{ background:colors.card, borderRadius:'16px', overflow:'hidden', border:`1px solid ${colors.border}`, cursor:'pointer' }}>
      ...
      <div style={{ padding:'14px' }}>
        ...
        <div style={{ display:'flex', gap:'4px', marginBottom:'10px' }}>
          <button onClick={e=>{e.stopPropagation();const msg=`🏠 *${property.title}*%0A💰 $${Number(property.price).toLocaleString('en-US')} | ${opLabels[property.operation_type]}%0A📍 ${property.zone}${property.address?' · '+property.address:''}%0A${property.bedrooms>0?'🛏 '+property.bedrooms+' hab · ':''}${property.bathrooms>0?'🚿 '+property.bathrooms+' baños · ':''}${property.area_m2?'📐 '+property.area_m2+'m²':''}%0A%0A📞 ${agent?.full_name||''} - ${agent?.phone||''}`;window.open('https://wa.me/?text='+msg,'_blank')}} style={{ flex:1, background:'#25D366', border:'none', color:colors.white, padding:'6px 8px', borderRadius:'6px', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:'4px', fontSize:'11px', fontWeight:'600' }}>📤 WhatsApp</button>
